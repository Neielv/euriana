<div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($permisos as $permiso)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"> {{ $permiso->servidor}}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    {{ $permiso->tipo}}
                                </td>                                
                                ...
                                <td class="px-6 py-4 whitespace-nowrap ext-sm font-medium flex">
                                    ...
                                    <a class="btn btn-red ml-2" wire:click="$emit('negarPermiso',{{ $permiso->id }})">
                                        <i class="fas fa-trash">

                                        </i>
                                    </a>   
                                    <a class="btn btn-red ml-2" wire:click="$emit('aprobarPermiso',{{ $permiso->id }})">
                                        <i class="fas fa-trash">
    ...
    @push('js')
        ...
        <script>
            Livewire.on('aprobarPermiso', PermisoId =>
                Swal.fire({
                    title: 'Está seguro de aprobar el permiso?' ,
                    text: "¡No podrás revertir esto!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Si, aprobar el permiso'
                }).then((result) => {
                    if (result.isConfirmed) {
                        Livewire.emitTo('show-aprobaciones', 'aprobar', PermisoId);
                        Swal.fire(
                        'Euriana!',
                        'Permiso aprobado.',
                        'success'
        ...
        <script>
            Livewire.on('negarPermiso', PermisoId =>
                Swal.fire({
                    title: 'Está seguro de negar el permiso?' ,
                    text: "¡No podrás revertir esto!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Si, negar el permiso'
                }).then((result) => {
                    if (result.isConfirmed) {
                        Livewire.emitTo('show-aprobaciones', 'negar', PermisoId);
                        Swal.fire(
                        'Euriana!',
                        'Permiso negado.',
                        'success'
